...
@njit(locals={'left_g_hf': float32, 'left_hx_hfx': float32,
              ...
              'n_samples_left': uint32},
      fastmath=True)
def _find_best_bin_to_split_helper_v2(context, feature_idx, histogram, n_samples):
    """Find best bin to split on, and return the corresponding SplitInfo.
    ...
    for bin_idx in range(context.n_bins_per_feature[feature_idx]):
        ...
        # TODO: add case with constant hessian

        left_g_hf += histogram[bin_idx]['sum_g_hf']
        right_g_hf = context.sum_g_hf - left_g_hf

        left_gx_hfx += histogram[bin_idx]['sum_gx_hfx']
        right_gx_hfx = context.sum_gx_hfx - left_gx_hfx

        left_h += histogram[bin_idx]['sum_h']
        right_h = context.sum_h - left_h

        left_hx += histogram[bin_idx]['sum_hx']
        right_hx = context.sum_hx - left_hx

        left_hx2 += histogram[bin_idx]['sum_hx2']
        right_hx2 = context.sum_hx2 - left_hx2
        ...
        if left_h < context.min_hessian_to_split:
            continue
        if right_h < context.min_hessian_to_split:
            # won't get any better (hessians are > 0 since loss is convex)
            break
        ...
        gain = _split_gain_v2(
            ...
            context.sum_g_hf, context.sum_gx_hfx, context.sum_h, context.sum_hx, context.sum_hx2,
...
@njit(fastmath=False)
def _split_gain_v2(left_g_hf, left_gx_hfx, left_h, left_hx, left_hx2,
                   right_g_hf, right_gx_hfx, right_h, right_hx, right_hx2,
                   sum_g_hf, sum_gx_hfx, sum_h, sum_hx, sum_hx2,
                   w_l2_reg, b_l2_reg):

    def negative_loss(g_hf, gx_hfx, h, hx, hx2):
        hx2_reg = hx2 + w_l2_reg
        h_reg = h + b_l2_reg
        return (hx2_reg * g_hf**2 - h_reg * gx_hfx**2) / (hx2_reg * h_reg - hx**2)

    gain = negative_loss(left_g_hf, left_gx_hfx, left_h, left_hx, left_hx2)
    gain += negative_loss(right_g_hf, right_gx_hfx, right_h, right_hx, right_hx2)
    gain -= negative_loss(sum_g_hf, sum_gx_hfx, sum_h, sum_hx, sum_hx2)
